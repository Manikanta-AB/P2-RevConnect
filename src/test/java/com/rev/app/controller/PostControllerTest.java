...
public class PostControllerTest {
    ...
    public void testSearchHashtag() throws Exception {
        ...
        when(userService.findByUsername(anyString())).thenReturn(user);
        when(postService.searchByHashtag(anyString())).thenReturn(Collections.emptyList());
        when(notificationService.getUnreadCount(anyLong())).thenReturn(0L);

        mockMvc.perform(get("/posts/search").param("hashtag", "test"))
                .andExpect(status().isOk())
                .andExpect(view().name("search-hashtag"))
                .andExpect(model().attributeExists("posts", "hashtag", "currentUser"));
    }
}
